...
# Direct evaluation of TEST_FOLDER
evaluate_saving = os.path.basename(TEST_FOLDER)
evaluate_saving_dir = TEST_FOLDER
action_history_dir = os.path.join(evaluate_saving_dir, 'action_history')
line_to_be_evaluated = collections.defaultdict(list)

def extract_number(response):
    """从 agent response 中提取数字"""
    import re
    # 移除 Python 列表格式的干扰
    # 找 "X rounds" 或 "X round" 模式
    round_patterns = [
        r'(\d+)\s*rounds?\s*(?:of\s+)?dialogue',
        r'(\d+)\s*rounds?\s*(?:of\s+)?conversation', 
        r'(\d+)\s*rounds?\s*(?:have\s+)?(?:been\s+)?(?:occurred|happened)?',
        r'there\s*(?:are|have|has|been)\s*(\d+)\s*rounds?',
    ]
    ...
    # 如果没找到，找 "lines / 2" 计算
    lines_match = re.search(r'(\d+)\s*lines?\s*(?:total|of|are)?', response, re.IGNORECASE)
    if lines_match:
        lines = int(lines_match.group(1))
        if 1 <= lines <= 40:
            return str(lines // 2)
    
    # 最后尝试：找 "equal X rounds" 或 "means X rounds"
    equal_match = re.search(r'equal\s*(?:to\s*)?(\d+)\s*rounds?', response, re.IGNORECASE)
    if equal_match:
        return equal_match.group(1)
...
if os.path.isdir(action_history_dir):
        ...
        # 修改比较逻辑，提取数字后再比较
        for j in line_to_be_evaluated:
            correct = 0
            total = 0
            for i in line_to_be_evaluated[j]:
                agent_num = extract_number(i[0])
                gt_num = i[1]
                if agent_num == gt_num:
                    correct += 1
                total += 1
            print(j, 'mean=', numpy.mean([1 if extract_number(i[0]) == i[1] else 0 for i in line_to_be_evaluated[j]]))

print('='*50)
print('Entropy Evaluation')
print('='*50)
print()
# Direct entropy evaluation of TEST_FOLDER
evaluate_saving = os.path.basename(TEST_FOLDER)
