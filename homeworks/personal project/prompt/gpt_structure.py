...
def GPT_request_by_url(message_or_prompt, gpt_params, url, key, log_dir, model='gpt-3.5-turbo', max_retries=5):
    ...
    for retry_count in range(max_retries):
        try:
            response = requests.request("POST", url, headers=headers, data=payload_json, timeout=120)
            response_json = response.json()
            
            # 检查响应是否有效
            if response_json and "choices" in response_json and len(response_json.get("choices", [])) > 0:
                break  # 成功获取有效响应
            else:
                print(f"[GPT_request_by_url] 响应无效，重试 {retry_count + 1}/{max_retries}...")
                response_json = {}
                
        except requests.exceptions.Timeout:
            print(f"API请求超时，重试 {retry_count + 1}/{max_retries}...")
            response_json = {}
        except Exception as e:
            print(f"[GPT_request_by_url] Error: {e}")
            response_json = {}
    ...
    try:
        ...
        if "choices" in write_json and len(write_json.get("choices", [])) > 0:
...
def deepseek_request(message_or_prompt, gpt_params, log_dir):
    key = deepseek_key
    url = deepseek_url
    return GPT_request_by_url(message_or_prompt, gpt_params, url, key, log_dir, model='deepseek-chat')
...
def generate(message_or_prompt, gpt_param=None, engine='gpt4', model=None, log_dir=None):
    ...
    for retry_count in range(max_retries):
        try:
            ...
            elif engine == 'gpt3.5' or engine == 'gpt3.5-turbo':
                response_json = GPT3_request(message_or_prompt, gpt_param, log_dir)
            elif engine == 'deepseek':
                response_json = deepseek_request(message_or_prompt, gpt_param, log_dir)
            elif engine == 'gpt4':
            ...
            if response_json and "choices" in response_json and len(response_json.get("choices", [])) > 0:
                ...
                if content:
                    ...
                    print(f"[generate] 返回内容为空，重试 {retry_count + 1}/{max_retries}...")
            else:
                print(f"[generate] 响应无效，重试 {retry_count + 1}/{max_retries}...")
                response_json = {}
                
        except Exception as e:
            print(f'[generate] Error (尝试 {retry_count + 1}/{max_retries}): {e}')
            print(response_json)
    ...
    # 所有重试都失败，返回空字符串而不是崩溃
    print(f"[generate] 警告: {engine} 请求失败次数过多，返回空字符串")
...
def get_embedding(text, retry=5):
    ...
    while retry > 0:
        try:
            response_json = requests.request("POST", url, headers=headers, data=payload_json, timeout=30).json()
