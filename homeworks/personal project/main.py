...
class AgentGroupChat:
    ...
    def save(self, save_folder) -> None:
        ...
        config_file = open('C:/Users/hp/OneDrive/Desktop/2/AgentGroup/AgentGroup-main/config.py', encoding='utf-8')
        basic_setting = {}
        ...
        for main_character_id_number in main_character_influence:
            ...
            for index, character in enumerate([main_character, chosen_character]):
                ...
                number_of_chat_round, thought, action_event = character.summarize(environment_description=environment_summary,
                                                                                  chat_history=chat_history)
                # 计算实际对话行数的一半（按 prompt 规则：两行 = 一轮）
                actual_chat_lines = len([line for line in chat_history.split('\n') if line.strip()])
                actual_chat_round = actual_chat_lines // 2
                evaluation_event = [character.id_number,
                                    character.id_number,
                                    '### EVALUATION CHAT ROUND',
                                    'agent response: %s[SEP]ground truth: %s' %
                                    (str(number_of_chat_round), str(actual_chat_round))]
                new_action_index = self.new_action_insert(evaluation_event, now_round_number)
                action_index.append(new_action_index)
                new_action_index = self.new_action_insert(action_event, now_round_number)
                action_index.append(new_action_index)
    ...
    def settlement_stage(self, whole_information, game_name='Succession'):
        ...
        # ====================================================== #
        # NoC and NoR Evaluation
        # ====================================================== #
        # 获取角色和资源的真实数量
        all_characters = self.characters.get_all_characters()
        num_characters_truth = len([c for c in all_characters if c.get_main_character()])
        num_resources_truth = len(self.resources.resource_list)

        # 对每个角色进行 NoC 和 NoR 评估
        for character in self.characters.character_list:
            if not character.get_main_character():
                continue

            # 获取角色已知的信息
            action_history = self.action_history.get_description(
                character_id_number=character.id_number,
                max_num=ACTIONHISTORY_RETRIEVE_NUM_PARTIAL_INFORMATION
            )

            # 让角色评估 NoC 和 NoR
            num_character_guess, num_resource_guess, thought = character.evaluate_noc_nor(
                environment_description='',
                action_history=action_history
            )

            # 记录 NoC 评估
            evaluation_event_noc = [
                character.id_number,
                character.id_number,
                '### EVALUATION NUMBER OF CHARACTER',
                'agent response: %s[SEP]ground truth: %s' % (str(num_character_guess), str(num_characters_truth))
            ]
            self.new_action_insert(evaluation_event_noc, -1)

            # 记录 NoR 评估
            evaluation_event_nor = [
                character.id_number,
                character.id_number,
                '### EVALUATION NUMBER OF RESOURCE',
                'agent response: %s[SEP]ground truth: %s' % (str(num_resource_guess), str(num_resources_truth))
            ]
            self.new_action_insert(evaluation_event_nor, -1)
            ...
            self.logger.gprint(
                ...
                source_character=character.id_number,
                target_character=character.id_number,
                log_type='NoC NoR Evaluation',
                log_content='NoC: agent=%s, truth=%s | NoR: agent=%s, truth=%s' % (
                    num_character_guess, num_characters_truth, num_resource_guess, num_resources_truth
                )
            )
