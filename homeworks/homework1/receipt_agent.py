...
6. Tax amount (税费)
7. Discount amounts (折扣金额) - shown with minus signs like -$20.00
8. Original amount BEFORE discounts (原价)
9. Payment method (支付方式) - cash, card, etc.
10. Merchant address (商家地址)

HOW TO READ RECEIPT AMOUNTS:
- Each receipt shows a FINAL TOTAL at the bottom
- Discounts are shown with a MINUS sign, e.g., '-$20.00', '-$5.50', '-¥100'
...
7. DISCOUNT AMOUNTS:
   - Sum up all discount values (remember they have minus signs)
...
def create_gradio_interface():
    """Create Gradio interface - Complete Receipt Analysis UI"""
    with gr.Blocks(title="🧾 Receipt Analysis Agent", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Row(equal_height=True):
            ...
            with gr.Column(scale=1, min_width=300):
                ...
                # Button row
                with gr.Row():
                    submit_btn = gr.Button("Submit", variant="primary", size="lg", scale=2)
                    clear_btn = gr.Button("Clear Chat", variant="stop", size="lg", scale=1)
        ...
        gr.Markdown("""
        - **Total spending**: "What is the total spending across all receipts?"
        - **Before discount**: "How much would I have paid without discounts?"
        - **Discount amount**: "What is the total discount amount?"
        - **Items purchased**: "What items did I purchase on each receipt?"
        - **Merchant names**: "What are the merchant names?"
        - **Highest spending**: "Which receipt has the highest spending?"
        - **Payment methods**: "What payment methods were used?"
        """)
